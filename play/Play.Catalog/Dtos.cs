...
namespace Play.Catalog
{
   ...
   public record CatalogItemCreate(Guid id, [Required][StringLength(100, ErrorMessage = "{0} should be greater than {1}")] string Name,
         string Description);

    public record CatalogItemUpdate(Guid id, [Required][StringLength(100, ErrorMessage = "{0} should be greater than {1}")] string Name,
        string Description);
